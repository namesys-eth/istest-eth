...
export default {
	chains: {
		"ethereum": ["https://rpc.ankr.com/eth", "https://eth-rpc.gateway.pokt.network"],
	...
	async handleRequest(request, env) {
		if (request.method !== 'GET') return this.Output("Method ${request.method} Not Allowed.", 405, 6);
		...
		if (paths.length < 4 || paths.length > 5) {
			return this.Output("Bad Request", 400, 6)
		}
		if (!this.chains[paths[1]]) {
			return this.Output(`${paths[1].toUpperCase()} Network Not Supported`, 400, 6);
		}
		...
		if (contentType.includes('application/json')) {
			...
			if (res.error || res.result === "0x") {
				return this.Output(res.error ? res.error.message : "Bad Request", 400, 6)
			}
			return this.Output(res.result, 200, 666);
		} else {
			return this.Output("Bad Gateway", 502, 7);
		}
	},
	Signed(result, calldata, env) {
		if (!env.PRIVATE_KEY) {
			return this.Output('Private Key Not Set.', 500, 6);
		}
