...
contract Resolver {
    ...
    function recoverSigner(
        ...
        /// @notice : first 32 bytes, after the length prefix
        bytes32 r = bytes32(signature[:32]);
        /// @notice : second 32 bytes
        bytes32 s = bytes32(signature[32:]) &
            bytes32(
                0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
            );
        /// @notice : final byte (first byte of the next 32 bytes)
        uint8 v = uint8((uint256(bytes32(signature[32:])) >> 255) + 27);
        if (
            uint256(s) >
    ...
    function __callback(
        ...
        if (!Resolver(address(this)).isValid(_digest, _signature))
